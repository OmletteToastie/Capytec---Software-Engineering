package taskentrycomponent; //Part of the Package taskentrycomponent
...
public class edittask extends JDialog {
	...
	public edittask() {
		setTitle("Edit Task Form"); //Sets the window title to be Edit Task Form.
		setModal(true);
		setBounds(100, 100, 779, 385);
		...
		//TASK ID SECTION.
		{
			...
			GridBagConstraints gbc_TaskIDLabel = new GridBagConstraints();
			gbc_TaskIDLabel.insets = new Insets(0, 0, 5, 5);
			gbc_TaskIDLabel.anchor = GridBagConstraints.EAST;
			gbc_TaskIDLabel.gridx = 3;
			gbc_TaskIDLabel.gridy = 0;
			tasknamedescpanel.add(TaskIDLabel, gbc_TaskIDLabel);
		}
		{
			TaskIDField = new JTextField();
			TaskIDField.setEditable(false);
			GridBagConstraints gbc_TaskIDField = new GridBagConstraints();
			gbc_TaskIDField.insets = new Insets(0, 0, 5, 0);
			gbc_TaskIDField.fill = GridBagConstraints.HORIZONTAL;
			gbc_TaskIDField.gridx = 4;
			gbc_TaskIDField.gridy = 0;
			tasknamedescpanel.add(TaskIDField, gbc_TaskIDField);
		...
		//DESCRIPTION LABELS FOR COMBOBOXES AND TEXT
		{
			...
			taskLengthField.addKeyListener(new KeyAdapter() {
		         public void keyPressed(KeyEvent ke) {
		        	 String taskLength = taskLengthField.getText();
			            int length = taskLength.length(); //Sets the text length of the field
		            
			            if ((ke.getKeyChar() >= '0' && ke.getKeyChar() <= '9' || (ke.getKeyChar()== KeyEvent.VK_BACK_SPACE)) && length<3) { //this will only accept numbers between 0 & 9 and backspaces, with a string length of 3
		            	taskLengthField.setEditable(true);
		            } else {
		            	taskLengthField.setEditable(true);
		            	 JOptionPane.showMessageDialog(rootPane, "Only Numbers Allowed"); //if the text is anything except in the if statement, this will display
		            }
		         }
			});
			
			taskLengthField.setText(" "); //Sets text as blank
		...
		{ //COMBOBOX FOR ROUTINE OR ONE OFF TASK
